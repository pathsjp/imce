 ...
 */
Drupal.behaviors.imceFileField = {attach: function(context, settings) {
  var i, button, $buttons = $('.imce-filefield-button', context).not('.iff-processed').addClass('iff-processed');
...
imceFileField.processButton = function(button) {
  var el, url = button.getAttribute('data-imce-url'), fieldId = button.id.replace('-imce-button', '');
  if (url && fieldId) {
...
imceFileField.sendto = function(File, win) {
  var imce = win.imce,
  items = imce.getSelection(),
  fieldId = imce.getQuery('fieldId'),
  exts = imceFileField.getFieldExts(fieldId);
  // Check extensions
...
imceFileField.getFieldExts = function(fieldId) {
  var settings = drupalSettings.file, elements = settings && settings.elements;

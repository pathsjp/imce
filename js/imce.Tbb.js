...
Tbb.construct = function(id, opt) {
  ...
  var el = Tbb.createEl(), parent = imce.toolbarEl;
...
Tbb.openPopup = function(autoclose) {
  ...
  if (!Tbb.active) {
    Tbb.createPopupEl();
    Tbb.setActive(true);
    var popupEl = Tbb.popupEl, $el = $(Tbb.el), css = $el.offset();
...
Tbb.toggleState = function(name, state) {
  var Tbb = this, oldState = Tbb[name];
  if (state == null) {
    state = !oldState;
  }
  if (state) {
...
imce.eTbbDocMousedown = function(e) {
  var Tbb = e.data.Tbb, el = Tbb.popupEl;

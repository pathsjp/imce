...
Item.remove = function(shallow) {
  if (this.parent) this.parent.removeItem(this, shallow);
};
...
Item.click = function(e) {
  ...
  if (e) {
    ...
    if (e.shiftKey) {
      ...
      lastItem = imce.getLastSelected(),
      start = lastItem ? Folder.indexOf(lastItem) : -1,
      end = Folder.indexOf(Item),
      step = start < end ? 1 : -1;
      while (start != end) {
  ...
  var i, selection = imce.getSelection();
  for (i in selection) {
    if (selection[i] !== Item) {
...
Item.getUrl = function(absolute, uncached) {
  ...
  if (rootUrl = imce.getConf('root_url')) {
    ...
    if (absolute && url.charAt(0) === '/' && url.charAt(1) !== '/') url = location.protocol + '//' + location.host + url;
    if (uncached) url += (url.indexOf('?') == -1 ? '?' : '&') + ('s' + this.size) + ('d' + this.date); 
  }
  return url;
};
...
Item.createPreviewEl = function() {
  var el, Item = this, prvEl = imce.createEl('<div class="imce-item-preview"></div>'),
  // Info
  infoEl = imce.createEl('<div class="imce-preview-info"></div>');

...
BUE.fileBrowsers.imce = function(field, type, E) {
  ...
  height = Math.min(800, parseInt(screen.availHeight * 0.8)),
  field_id = BUE.imce.fields.length, 
  url = BUE.imce.url('sendto=BUE.imce.sendtoField&type=' + type + '&field_id=' + field_id);
...
BUE.imce = BUE.imce || {
  ...
  sendtoField: function(File, win) {
    var field, id = win.imce.getQuery('field_id');
    if (field = BUE.imce.fields[id]) {
      // Set field value
      field.value = File.getUrl();
      // Check other fields
      var name, input, value, values = {width: File.width, height: File.height, alt: File.formatName()};
      for (name in values) {

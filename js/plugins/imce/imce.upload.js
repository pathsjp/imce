...
imce.createUploadForm = function() {
  var els, el, form = imce.uploadForm, name = 'files[imce][]';
  if (form) return form;
...
imce.validateUploadForm = function(form) {
  var i, file, input = form.elements['files[imce][]'], files = input.files;
  // HTML5
  if (files) {
    if (!files.length) {
      return false;
...
imce.validateFileUpload = function(file) {
  ...
  if (maxsize && file.size && file.size > maxsize) {
    imce.setMessage(Drupal.t('%filename is %filesize exceeding the maximum file size of %maxsize.', {'%filename': file.name, '%filesize': imce.formatSize(file.size), '%maxsize': imce.formatSize(maxsize)}));
    return false;
  }
  // Name
  ...
  // Trigger validators.
  if ($.inArray(false, imce.trigger('validateFileUpload', file)) !== -1) {
    return false;
  }
  return true;
};
...
imce.createUploadIframe = function() {
  ...
  if (!el) {
    ...
    setTimeout(function() {el.onload = imce.eUploadIframeLoad});
  }
  return el;
};
...
imce.eUploadIframeLoad = function(event) {
  var text, response, doc, $body, el = this;
  try {
...
imce.uploadIframeComplete = function(response, text) {
  ...
  // Failed
    ...
    if (text) imce.setMessage('<pre>' + Drupal.checkPlain(text) + '</pre>');
  }

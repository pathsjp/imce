...
Uq.createEl = function() {
  var Uq = this, el = Uq.el, inputEl, accept;
  if (!el) {
    ...
    if (accept = Uq.conf.accept) inputEl.accept = accept;
 ...
 */
Uq.next = function() {
  var i, Item, queue = this.queue.slice(0);
...
Uq.selectFiles = function(list) {
  var i, file, Item, ret, path = imce.activeFolder.getPath();
...
Uq.addItem = function(Item) {
  var existing, Uq = this, id = Item.id;
  // Check existing
...
Uq.removeItem = function(Item, quick) {
  var Uq = this, queue = Uq.queue, i = $.inArray(Item, queue);
  if (i != -1) {
    queue.splice(i, 1);
...
Uq.ajaxPrepare = function(Item) {
  var i, field, data, formData, Folder, Uq = this, file = Item.file, dest = Item.destination;
  // Check file and destination
  if (!file || !dest || !(Folder = imce.getFolder(dest))) return;
  // Prepare form data
...
UqItem.createEl = function() {
  var i, cancelEl, name, Item = this, el = Item.el, file = Item.file;
  if (!el) {
...
UqItem.remove = function(quick) {
  var ret, Item = this, Uq = Item.Uq;
  Item.stop();
  if (Uq) Uq.removeItem(Item, quick);
...
UqItem.complete = function(status) {
  ...
  if (!Item.completed) {
    ...
    if (status) $('.imce-uqi-percent', Item.el).html(Drupal.t('!percent%', {'!percent': 100}));
    // Check if this is the active item of the queue
    if (Uq && Uq.activeItem === Item) {
      Uq.activeItem = null;
      // Continue queue
      if (Uq.running) Uq.next();
    }
...
imce.xUqItemBeforeSend = function(xhr) {
  ...
  xhr.upload.onprogress = function(e) {
    ...
    if (Item) Item.progress(parseInt(e.loaded * 100 / e.total));
  };
  xhr = null;
};
...
imce.xUqItemComplete = function(xhr, status) {
  var opt = this, Item = imce.activeUq.getItem(opt.itemId), status = !!(opt.response && opt.response.added);
  if (Item) Item.complete(status);
};

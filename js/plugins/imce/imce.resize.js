...
imce.eResizeSubmit = function() {
  var data, els = this.elements,
  width = parseInt(els.width.value * 1),
  height = parseInt(els.height.value * 1),
  copy = els.copy.checked ? 1 : 0,
...
imce.eResizeInputFocus = function() {
  ...
  setTimeout(function() {
    if (el === document.activeElement && val === el.value) {
      var ratio, els = el.form.elements,
      isWidth = els.width === el,
      value = els[isWidth ? 'height' : 'width'].value * 1,
      Item = imce.previewingItem;

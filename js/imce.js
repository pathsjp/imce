...
$(document).ready(function() {
  var settings = window.drupalSettings, conf = settings && settings.imce, body = document.body;
...
imce.init = function(conf, parentEl) {
  ...
  if (!conf.ajax_url || !conf.folders || !conf.root_url) return;
  ...
  // Open active path
  var path = conf.active_path, Folder = path && imce.addFolder(path);
  if (!Folder) {
...
imce.initUI = function() {
  var el = imce.fmEl, createEl = imce.createEl;
  if (el) return el;
...
imce.initTree = function() {
  var path, Folder, folders = imce.getConf('folders'),
  // Create root
  scheme = imce.getConf('scheme'), 
  root = new imce.Folder(scheme ? scheme + '://' : '<' + Drupal.t('root') + '>');
  root.setPath('.');
...
imce.getItem = function(path) {
  var Folder, parts = imce.splitPath(path);
  if (parts) {
...
imce.addFolder = function(path, conf) {
  var parts, parent, Folder = imce.getFolder(path);
  // Existing
  if (Folder) {
    if (conf) Folder.setConf(conf);
    return Folder;
  }
...
imce.getLastSelected = function() {
  var arr = imce.selection, len = arr.length;
...
imce.selectItem = function(Item) {
  if (!Item.selected) {
    var arr = imce.selection, oldlen = arr.length;
    arr.push(Item);
    Item.setState('selected');
    if (oldlen < 2) imce.updatePreview();
  }
};
...
imce.deselectItem = function(Item) {
  if (Item.selected) {
    var arr = imce.selection, i = $.inArray(Item, arr);
    Item.unsetState('selected');
    if (i !== -1) {
      arr.splice(i, 1);
      if (arr.length < 2) imce.updatePreview();
    }
  }
};
...
imce.checkIntegration = function() {
  var query = imce.getQuery(), handler, urlField, initHandler, parentWin = window.opener || window.parent;
...
imce.createSendtoTbb = function(title, desc) {
  if (imce.sendtoHandler && !imce.getTbb('sendto')) {
    return imce.addTbb('sendto', {
      ...
      handler: function() {imce.runSendtoHandler()},
      icon: 'check'
    });
  }
};
...
imce.runSendtoHandler = function(items) {
  var Item, prop, handler = imce.sendtoHandler;
  if (handler) {
...
imce.defaultActivateFolder = function(Folder, oldFolder) {
  ...
  var i, j, Tbb, perm, disabled, buttons = imce.toolbarButtons;
  for (i in buttons) {
    Tbb = buttons[i];
...
imce.updateHeader = function() {
  var newsort = imce.activeFolder.activeSort, oldsort = imce.activeSort || {}, el = imce.contentHeaderEl;
  // Check if the sort has changed.
  if (newsort && (newsort.key !== oldsort.key || newsort.desc !== oldsort.desc)) {
    // Deactivate existing column
    if (oldsort.key) $('[data-sort="' + oldsort.key + '"]', el).removeClass('sorted ' + (oldsort.desc ? 'desc' : 'asc'));
    // Activate new column
...
imce.groupItems = function(items) {
  var i, Item, path, selected, key, names, group = {};
  for (i in items) {
    Item = items[i];
...
imce.validatePermissions = function(items, filePerm, subfolderPerm) {
  var path, Folder, selected, groups = imce.groupItems(items);
  for (path in groups) {
...
imce.validateFileName = function(name) {
  // Basic validation
  if (!name || name === '.' || name === '..' || !name.length || name.length > 240) return false;
  // Test name filters
...
imce.validateDimensions = function(items, width, height) {
  ...
  var maxwidth = imce.getConf('maxwidth'), maxheight = imce.getConf('maxheight');
...
imce.eTreeUp = function(e) {
  var Folder = imce.activeFolder, prvEl, prvFolder;
...
imce.eTreeDown = function(e) {
  var Folder = imce.activeFolder, nextEl, nextFolder;
...
imce.eContentHeaderClick = function(event) {
  var key, e = imce.eFix(event), Folder = imce.activeFolder, sort = Folder.activeSort || {};
...
imce.eContentArrow = function(e) {
  var Item, i = 0, Folder = imce.activeFolder, key = e.keyCode;
...
imce.eBodyResizerDrag = function(e) {
  var el = imce.bodyResizerEl, bodyH = Math.min(Math.max(el.startH + e.pageY - el.startY, 0), el.maxH);
...
imce.eWinResize = function(e) {
  var pdiff, diff = imce.getWindowSize().height - imce.fmEl.offsetHeight;
  // Distribute the excess space to the body and preview elements.
  if (diff) {
    var $bodyEl = $(imce.bodyEl), $prvEl = $(imce.previewEl);
...
imce.bind = function(type, handler) {
  var events = imce.events, handlers = events[type];
...
imce.trigger = function(type) {
  var i, handler, handlers = imce.events[type], ret = [];
  if (handlers) {
    for (i in handlers) {
...
imce.eFireShortcut = function(event, area) {
  var e = event || window.event, shortcut = imce.eBuildShortcut(e);
...
imce.ajaxPrepare = function(jsop, opt) {
  // Prepare data
  var path, Folder = opt && opt.activeFolder != null ? opt.activeFolder : imce.activeFolder,
  data = {jsop: jsop, token: imce.getConf('token')};
  if (Folder) {
...
imce.ajaxBeforeSend = function(xhr, opt) {
  ...
  if (Folder = opt.activeFolder) Folder.setBusy(true);
};
...
imce.ajaxProcessRemoved = function(response) {
  var i, Item, paths = response.removed;
  if (paths) {
    for (i in paths) {
...
imce.setMessage = function(msg, type) {
  if (!type) type = 'error';
  var mq = imce.messageQueue, len = mq.length, msgId = msg + ':' + type;
  ...
  if (!imce.pmqScheduled) imce.pmqScheduled = setTimeout(imce.processMessageQueue, 100);
  return false;
};
...
imce.mPopClose = function() {
  ...
  if (imce.mPopHovering) return imce.mPopCloseTimerUp = 1;
...
imce.processMessageQueueNext = function() {
  imce.pmqScheduled = 0;
  if (imce.messageQueue.length) imce.pmqScheduled = setTimeout(imce.processMessageQueue, 250);
};
...
imce.eMPopMouseenter = function(e) {
  ...
  if (imce.mPopCloseTimerUp) clearTimeout(imce.mPopCloseTimer);
};
...
imce.eMPopMouseleave = function(e) {
  ...
  if (imce.mPopCloseTimerUp) imce.mPopCloseTimer = setTimeout(imce.mPopClose, 2000);
};
...
imce.eMPopDocMousedown = function(e) {
  ...
  if (!imce.mPopHovering) imce.mPopClose();
};
...
imce.hasPermission = function(permission, conf) {
  var i, folders = (conf || imce.conf).folders;
  if (folders) {
    for (i in folders) {
...
imce.sortNumericProperty = function(a, b, prop) {
  ...
  return a.isFolder === b.isFolder ? imce.sortNum(a[prop]||0, b[prop]||0) : (a.isFolder ? -1 : 1);
};
...
imce.sorters.name = function(a, b) {
  return a.isFolder === b.isFolder ? imce.sortText(a.name, b.name) : (a.isFolder ? -1 : 1);
};
...
imce.sorters.ext = function(a, b) {
  return a.isFolder === b.isFolder ? (a.isFolder ? 0 : imce.sortText(a.ext||'', b.ext||'')) : (a.isFolder ? -1 : 1);
};
...
imce.getQuery = function(name) {
  var i, part, parts, str, query = imce.query;
  if (!query) {
    query = imce.query = {};
...
imce.formatDate = function(timestamp, dayOnly) {
  var D, p0, ret = '';
  if (timestamp) {
    D = new Date(timestamp * 1000), p0 = imce.prependZero;
    ret = D.getFullYear() + '-' + p0(D.getMonth() + 1) + '-' + p0(D.getDate());
    if (!dayOnly) ret += ' ' + p0(D.getHours()) + ':' + p0(D.getMinutes());
  }
  return ret;
};
...
imce.formatSize = function(size) {
  if (size == null) return '';
  if (!size || size < 100) return Drupal.formatPlural(size, '1 byte', '@count bytes', {'@count': size || 0});
  if (size < 1048576) return Drupal.t('@size KB', {'@size': imce.round(size/1024, 1)});
...
imce.formatItemsStatus = function(count, size) {
  return Drupal.t('!items (!size)', {'!items': Drupal.formatPlural(count, '1 item', '@count items'), '!size': imce.formatSize(size)});
};
...
imce.resolveHandler = function(str, scope) {
  if (!str) return;
  var i, obj = scope || window, parts = str.split('.'), len = parts.length;
...
imce.createEl = function(html) {
  var el, div = imce._div;
  if (!div) div = imce._div = document.createElement('div');
  div.innerHTML = html;
  el = div.firstChild;
  div.removeChild(el);
  return el;
...
imce.scrollToEl = function(el, wrpEl, diffTop, diffBottom) {
  if (el.offsetWidth && wrpEl.scrollHeight > wrpEl.clientHeight) {
    ...
    wrpTop = $(wrpEl).offset().top,
    wrpBottom = wrpTop + wrpEl.offsetHeight;
    wrpTop += diffTop || 0;
...
imce.fixPosition = function(el) {
  var pos = $(el).offset(), winSize = imce.getWindowSize(), winScroll = imce.getWindowScroll(), scrollbar = 18,
  extraX = pos.left - winScroll.left + el.offsetWidth - winSize.width + scrollbar,
  extraY = pos.top - winScroll.top + el.offsetHeight - winSize.height + scrollbar;
  // Shift to left
  if (extraX > 0) {
...
imce.eDocDrag = function(e) {
  ...
  if (edata.drag) {
    // Fix touch event
    if (edata.isTouch) e = imce.eTouchFix(e, e.originalEvent.changedTouches[0]);
...
imce.eCommonTouchstart = function(event, callback, context) {
  ...
  if (touches && (touch = touches[0]) && !touches[1]) {
    if (callback && callback.call) return callback.call(context || this, imce.eTouchFix(event, touch), true);
    // Prevent default.
    return false;
  }
};

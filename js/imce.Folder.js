...
Folder.createEl = function() {
  var nameEl, toggleEl, branchEl, Folder = this;
  if (!Folder.el) {
    // Item elements.
...
Folder.setContent = function(content) {
  var i, Item, list, Folder = this, items = Folder.getItems();
...
Folder.addContent = function(content, selectNew) {
  var Folder = this, files = content.files, subfolders = content.subfolders;
  if (!files && !subfolders) return;
  // Add items
  Folder.addItems(files, 'file');
  Folder.addItems(subfolders, 'folder');
  // Update sort
  if (Folder.active) Folder.sortItems();
  else Folder.needSort = 1;
  Folder.sortTree();
  // Select new items.
  if (selectNew && Folder.active) {
    var name, fname, sname;
    imce.deselectAll();
    if (files) for (fname in files) {
      Folder.getItem(fname).select();
    }
    if (subfolders) for (sname in subfolders) {
...
Folder.addItems = function(items, type) {
  var Item, name, Type = type === 'folder' ? imce.Folder : imce.File;
  if (items) {
    for (name in items) {
      // Update
...
Folder.appendItem = function(Item) {
  var Folder = this, name = Item.name, parent = Item.parent, existing;
  if (!Folder.validateAppend(Item)) return;
...
Folder.removeItem = function(Item, shallow, i) {
  var i, name = Item.name, Folder = this;
  // Check if the item is a child
  if (Item.parent !== Folder) return;
  // Deep removal
  if (!shallow) {
  ...
  // Remove subfolder
  if (Item.isFolder) {
    if (Item.active) Folder.activate();
    Item.setPath(null);
  ...
  // Remove item
  Folder.items.splice(i === undefined ? Folder.indexOf(Item) : i, 1);
  delete Item.parent;
...
Folder.setPath = function(newpath) {
  var i, Folder = this, oldpath = Folder.path, subfolders = Folder.subfolders;
  if (oldpath != newpath) {
    // Remove
    if (newpath == null) {
      for (i in subfolders) {
...
Folder.open = function(refresh) {
  if (refresh || !this.content) this.load();
  this.activate();
};
...
Folder.activate = function() {
  var i, Folder = this, oldFolder = imce.activeFolder, parent = Folder.parent;
  if (!Folder.active) {
    // Deactivate the old dir.
    if (oldFolder) oldFolder.deactivate();
    // Check sort
    if (Folder.needSort) {
      Folder.sortItems();
    }
...
Folder.countSubfolders = function() {
  var i, count = 0;
  for (i in this.subfolders) count++;
  return count;
};
...
Folder.onItemNameChange = function(Item, oldname) {
  var Folder = this, name = Item.name, group = Item.isFolder ? Folder.subfolders : Folder.files;
...
Folder.addContentToDom = function() {
  var el = this.contentEl, parentEl = imce.contentEl;
...
Folder.updateStatus = function() {
  if (this.active) imce.updateStatus();
};
...
Folder.updateHeader = function() {
  if (this.active) imce.updateHeader();
};
...
Folder.sortItems = function(key, desc) {
  var i, sorter, Folder = this, items = Folder.items,
  active = Folder.activeSort || imce.activeSort || imce.local.activeSort || {};
  if (key == null) key = active.key || 'name';
  if (desc == null) desc = !!active.desc;
...
Folder.sortTree = function() {
  var i, Item, Folder = this, subfolders = Folder.subfolders, arr = [];
  for (i in subfolders) {
...
Folder.validateAppend = function(Item, copy) {
  ...
  if (!copy && parent === this) return false;
...
Folder.hasPredefinedPath = function() {
  if (this.isPredefined()) return this;
  var i, Folder, subfolders = this.subfolders;
  for (i in subfolders) {
...
Folder.getSize = function() {
  var i, size = 0, files = this.files;
  for (i in files) size += files[i].size || 0;
  return size;
}
...
imce.eBranchToggleClick = function(event) {
  ...
  if (Folder.countSubfolders()) {
    Folder.expanded ? Folder.shrink() : Folder.expand();
  }
  else {
    Folder.open();
  }
...
imce.xFolderLoadComplete = function(xhr, status) {
  var content, opt = this, Folder = opt.activeFolder, response = opt.response;
  if (response && (content = response.content)) {
    Folder.setContent(content);
  }
  Folder.setLoading(false);
  if (Folder.countSubfolders()) Folder.expand();
};
